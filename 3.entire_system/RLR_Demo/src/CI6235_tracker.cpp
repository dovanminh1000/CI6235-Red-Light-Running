        ...
        //for (const auto &trk : tracks)
        {
            if (0 == association[i][j])
            {
                ...
                if (iou_matrix[i][j] >= iou_threshold)
                {
                    ...
                    matched_flag = true;
                    trk.second.detection = detection[i];
                    //tracks[0].
                }
...
void Tracker::Run(const std::vector<cv::Rect> &detections)
{
    ...
    for (const auto &match : matched)
    {
        ...
        cv::Point xy = cv::Point(tracks_[ID].detection.x, tracks_[ID].detection.y + tracks_[ID].detection.height / 2); //CI6235: Get central point of current bounding box of vehicle
        tracks_[ID].his_xy.push_back(xy);
        int his_xy_size = tracks_[ID].his_xy.size();
        if (his_xy_size > 0)
        {
            if (tracks_[ID].enable_license_plate == false)
            {
                tracks_[ID].get_license_plate = false;
            }
            //CI6235: Check vehicle appearing near the camera to take evidence of violation vehicle, only take 1 picture for each violation vehicle
            if (tracks_[ID].enable_license_plate && tracks_[ID].his_xy[his_xy_size - 1].x < 50 && tracks_[ID].his_xy[0].x - tracks_[ID].his_xy[his_xy_size - 1].x > 10)
            {
                tracks_[ID].enable_license_plate = false;
                tracks_[ID].get_license_plate = true;
            }
        }
    }
